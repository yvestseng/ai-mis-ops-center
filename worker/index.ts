...
const worker = {
  async fetch(
    ...
    try {
      ...
      const priorityRuleMatch = url.pathname.match(
        /^\/api\/admin\/priority-rules(?:\/([^/]+))?$/,
      );
      if (priorityRuleMatch) {
        return respond(
          handlePriorityRuleRequest(request, env.DB, priorityRuleMatch[1]),
        );
      }
      ...
      const supportTeamMatch = url.pathname.match(
        /^\/api\/support-teams\/([^/]+)\/members$/,
      );
      if (supportTeamMatch) {
        return respond(
          handleSupportTeamRequest(request, env.DB, supportTeamMatch[1]),
        );
      }
