...
test(
  "company-wide ERP login outage is normalized as a P1 business application outage",
  () => {
    ...
    assert.equal(
      expectedPriority(workType, impact),
      "P1",
    );
  },
);
...
test(
  "ERP boundary cases do not promote ordinary or request cases to P1",
  () => {
    const cases = [
      {
        input: "我的 ERP 程式錯誤無法使用",
        priority: "P3",
      },
      {
        input: "財務部ERP無法登入",
...
test(
  "generic company-wide login outage preserves identity routing",
  () => {
    const input = "全公司無法登入";
