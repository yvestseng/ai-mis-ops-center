...
type LoadKey = "users" | "roles" | "audit" | "teams";
...
function statusMessage(status: number) {
  ...
  if (status === 403) return "權限不足，無法執行此操作。";
  if (status === 409) return "資料已存在或狀態衝突，請重新整理後再試。";
...
function errorMessage(error: unknown) {
  if (error instanceof ApiError) {
    return error.traceId ? `${error.message}（追蹤 ID：${error.traceId}）` : error.message;
  }
  ...
  return error instanceof Error ? error.message : "資料處理失敗。";
}
...
async function api(path: string, init?: RequestInit) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new ApiError(result.message || statusMessage(response.status), {
...
export function RbacConsole() {
  ...
  const load = useCallback(async () => {
    ...
    } satisfies Record<LoadKey, Promise<ApiResult>>;
    const entries = await Promise.allSettled(Object.entries(requests).map(async ([key, request]) => [key, await request] as const));
    const nextErrors: Partial<Record<LoadKey, string>> = {};

    for (const entry of entries) {
      if (entry.status === "fulfilled") {
        const [key, result] = entry.value;
        if (key === "users") setUsers(result.items || []);
        if (key === "roles") setRoles(result.items || []);
        if (key === "audit") setAuditItems(result.items || []);
        if (key === "teams") setTeams(result.items || []);
        continue;
      }

      nextErrors.users = nextErrors.users || errorMessage(entry.reason);
    }
  ...
  const sectionError = (key: LoadKey) => loadErrors[key] ? <div className="empty-state"><b>資料載入失敗</b><span>{loadErrors[key]}</span><button className="secondary" onClick={() => void load().catch((error) => setMessage(error.message))}>重試</button></div> : null;
  ...
  async function resetPassword(event: React.FormEvent) {
    ...
    try {
      ...
      await load(); flash(result.message || "密碼已重設；使用者下次登入必須變更密碼。");
    } catch (error) {
      flash(errorMessage(error));
    } finally { setBusy(false); }
  }
