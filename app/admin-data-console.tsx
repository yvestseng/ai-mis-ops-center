...
export function RbacConsole() {
  ...
  return <section className="management-console">
    ...
    {tab === "users" && <div className="card data-table"><div className="account-auth-note"><ShieldCheck size={18}/><span><b>使用者與角色管理</b><small>可使用 Enable／Disable 管理帳號登入資格；停用會立即撤銷帳號的既有工作階段。維運團隊及派工資格請至「派工成員」管理。</small></span></div><table><thead><tr><th>使用者</th><th>登入帳號</th><th>部門</th><th>角色</th><th>密碼狀態</th><th>狀態</th><th>最近登入</th><th>操作</th></tr></thead><tbody>{users.map((user) => { const isEnabled = user.status === "active"; const isUpdating = updatingUserId === String(user.id); return <tr key={String(user.id)}><td><b>{String(user.displayName)}</b><small>{String(user.email)}</small></td><td><b>{String(user.username || "尚未設定")}</b></td><td>{String(user.department || "未設定")}</td><td><select value={String(user.roleId)} disabled={isUpdating} onChange={(event) => void updateUser(String(user.id), { roleId: event.target.value })}>{roles.map((role) => <option key={String(role.id)} value={String(role.id)}>{String(role.name)}</option>)}</select></td><td><span className={`sso-pill ${user.hasPassword ? "ready" : ""}`}>{!user.hasPassword ? "待重設" : user.mustChangePassword ? "首次登入需變更" : "可登入"}</span></td><td><span className={`state-pill ${isEnabled ? "good" : ""}`}>{isEnabled ? "已啟用" : "已停用"}</span></td><td>{user.lastLoginAt ? new Date(String(user.lastLoginAt)).toLocaleString("zh-TW") : "待首次登入"}</td><td><div className="row-actions"><button className={`account-status-action ${isEnabled ? "disable" : "enable"}`} disabled={isUpdating} aria-label={`${isEnabled ? "停用" : "啟用"} ${String(user.displayName)}`} title={isEnabled ? "Disable 帳號" : "Enable 帳號"} onClick={() => void toggleUserStatus(user)}>{isUpdating ? "處理中…" : isEnabled ? "Disable" : "Enable"}</button><button disabled={isUpdating} aria-label="重設密碼" title="重設密碼" onClick={() => { setPasswordTarget(user); setResetUsername(String(user.username || user.email || "").split("@")[0].toLowerCase()); setNewPassword(""); setModal("password"); }}><Pencil size={15}/></button><button disabled={isUpdating} aria-label="刪除使用者" title="刪除使用者" onClick={() => void removeUser(String(user.id))}><Trash2 size={15}/></button></div></td></tr>; })}</tbody></table></div>}
